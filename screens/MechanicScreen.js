...
import ProviderRecents from "../components/ProviderRecents";
...
const MechanicScreen = () => {
  ...
  useEffect(() => {
    ...
    history?.map((item, index) => {
      if (index % 2 === 0) {
        newerArr.push(history.slice(index, index + 2));
      }
    });
  ...
  async function getHistoryData(array) {
    ...
    const queryData = await (
      await getDocs(historyQueryRef, where("id", "array-contains-any", array))
    ).docs;
    queryData.map((data) => setHistory((y) => [...y, data.data()]));
    console.log("Data of history ", queryData);
  }
  ...
  function submitData() {
    if ((name && priceRange && timeOpen && daysOpen && services) !== null) {
      const mydata = {
        name: name,
        daysOpen: daysOpen,
        timeOpen: timeOpen,
  ...
  return (
    ...
    >
      <View
        style={[
          styles.divideViews,
          tw`bg-yellow-400 rounded-bl-3xl mb-1 h-1/2`,
          { paddingTop: notch },
        ]}
      >
        <MechanicProfile
      ...
      <View style={tw`mt-5 h-1/2`}>
        <View style={tw`h-full`}>
          <View style={tw`flex flex-row justify-evenly items-center my-2`}>
            <Text style={tw`text-3xl font-extrabold`}>Recent projects</Text>
            <View>
              <TouchableOpacity
              ...
              >
                <Text style={tw`text-xl font-bold`}>Sign Out</Text>
              </TouchableOpacity>
            </View>
          </View>
          <ScrollView>
            {newArr &&
              newArr.map((x, index) => (
                <View key={index} style={[tw`flex-row my-1`]}>
                  {x.map((y, index) => (
                    <ProviderRecents data={y} key={index}/>
                  ))}
                </View>
              ))}
          </ScrollView>

          {/* We have a moda that is used when creating the profile details */}
          <Modal
            transparent={true}
            visible={isVisible}
            animationType="slide"
          ...
          >
            ...
            >
              <TouchableOpacity
                ...
                style={[
                  tw`h-12 w-24 rounded-xl flex flex-row items-center justify-center mx-6 bg-yellow-400`,
                  {
                    marginBottom: -26,
                    zIndex: 12,
                  },
              ...
              <View
                style={[
                  tw`h-5/6 pt-10 w-full flex flex-col items-center `,
                  { backgroundColor: "#f3f3f3" },
                ]}
              >
                <ScrollView>
                  <Text style={tw`text-4xl font-bold my-4`}>
                  ...
                  <View style={[tw`w-full`]}>
                    <Text style={tw`font-bold my-2`}>
                      Enter service below in the following format, name, price
                      in ksh., description, separated with commas.
                    </Text>
                    <TextInput
                      style={styles.textInput}
                      placeholderTextColor="#000"
                  ...
                  >
                    <TouchableOpacity
                      ...
                      style={tw`w-40 mx-2 flex justify-center items-center  bg-yellow-400 h-12 rounded-xl`}
                    >
                      <Text style={tw`text-base font-bold`}>Add a service</Text>
                    </TouchableOpacity>
...
const styles = StyleSheet.create({
  ...
  textInput: [
    tw`w-full rounded-md h-12 px-4 mb-4`,
    {
      backgroundColor: "#fff",
    },
  ],
